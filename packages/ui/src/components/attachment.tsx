...
/**
 * File chip for composer drafts and delivered messages. `state` drives the
 * upload lifecycle styling; `idle` renders as a dashed drop target.
 */
function Attachment({
  className,
  state = "done",
...
/**
 * Invisible overlay that makes the whole chip open or select the file while
 * `AttachmentActions` keep their own click targets above it.
 */
...
function AttachmentGroup({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex min-w-0 snap-x snap-mandatory scroll-px-1 [scrollbar-width:none] gap-3 overflow-x-auto overscroll-x-contain py-1 *:data-[slot=attachment]:flex-none *:data-[slot=attachment]:snap-start",
        className
      )}
      {...props}
    />
