...
portable request. Every page includes the exact `totalSize` of the matching records visible to the
caller before pagination. Object actions use `/leads/{id}/actions/qualify`; collection actions use
`/leads/actions/qualify`. Filters are typed by property kind, compose with `and`, `or`, and `not`,
and sorting is ordered and deterministic:
