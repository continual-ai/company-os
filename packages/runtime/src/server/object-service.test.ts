...
import * as ObjectService from "#/server/object-service.ts"
...
describe("ObjectService", () => {
  ...
  it("provides validated CRUD, atomic batching, pagination, and optimistic writes", async () => {
    ...
    const writer = ObjectService.makeWriter(Account, repository, {
      generateRecordId: () => `account_${++nextId}`,
      rootId,
      resolveRecordAliases: (_expectedType, aliases) => {
        aliasResolutionRequests.push(aliases)
    ...
    const result = await Effect.runPromise(
      Effect.gen(function* () {
        ...
        const overlappingAliasUpdate = yield* service
          ...
          .pipe(Effect.flip)
        const writerOutput = yield* writer.update({
          id: first.id,
          syncResult: "complete",
        })
      ...
      }).pipe(Effect.provideService(CurrentInvocation, context))
    )
    ...
    expect(result.writerOutput.syncResult).toBe("complete")
