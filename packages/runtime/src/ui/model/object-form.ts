...
import type { FormValue, FormValueObject } from "#/ui/model/form-value.ts"
import {
  modelObjectProperty,
  type ClientRecord,
  type ClientValue,
...
import { type ModelUiRuntime } from "#/ui/model/runtime-context.tsx"
...
export function decodeObjectForm(
  ...
  for (const { id, property, schema } of objectFormProperties(object, mode)) {
    ...
    const value = scalarValue(runtime, values, id, property, schema, mode)
  ...
  const schema =
    mode === "create"
      ? toEffectModelObjectCreateSchema(runtime.model, object)
      : toEffectModelObjectUpdateSchema(runtime.model, object)
  let decoded: unknown
  try {
