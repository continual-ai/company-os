...
import { defineRoot } from "#/model/definition/root.ts"
import {
  RecordAlias,
  RecordId,
  schema,
...
const Account = defineObject({
  ...
  properties: {
    ...
    syncResult: schema.string({ nullable: true, outputOnly: true }),
  },
...
describe("Effect Schema projection", () => {
  it("canonicalizes semantic string inputs before validation", () => {
    ...
    expect(decodeDomain("  EXAMPLE.Example  ")).toBe("example.example")
  })
  ...
  it("decodes records from portable object definitions", () => {
    ...
    expect(
      decode({
        ...
        logo: { assetId: "asset_1", alt: "Example logo" },
        name: "Example",
        status: "active",
      })
    ).toEqual({
  ...
  it("derives create and update inputs from object properties", () => {
    ...
    type WriterUpdate = ObjectWriterUpdateInput<typeof Account>
    ...
    expectTypeOf<Update["status"]>().toEqualTypeOf<
      "active" | "inactive" | undefined
    >()
    expectTypeOf<Create>().not.toHaveProperty("syncResult")
    expectTypeOf<Update>().not.toHaveProperty("syncResult")
    expectTypeOf<WriterUpdate["syncResult"]>().toEqualTypeOf<
    ...
    const decodeWriterUpdate = Schema.decodeUnknownSync(
      toEffectObjectWriterUpdateSchema(Account)
    )
    ...
    expect(decodeUpdate({ syncResult: "complete" })).toEqual({})
    expect(decodeWriterUpdate({ syncResult: "complete" })).toEqual({
      syncResult: "complete",
    })
