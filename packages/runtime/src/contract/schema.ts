...
import type { ApiError, ErrorType } from "#/model/definition/error.ts"
import {
  type ModelCatalog,
...
import type { PropertyDefinition } from "#/model/definition/property.ts"
import type {
  AnySchema,
  DecimalSchema,
  InferSchema,
...
function compileUpdateProperties(
  object: ObjectType,
  includeOutputOnly = false
): CompiledSchemaFields {
  return Object.fromEntries(
    Object.entries(object.properties)
      .filter(([, property]) => includeOutputOnly || !property.outputOnly)
      .map(([propertyId, property]) =>
        entry(
          propertyId,
          Schema.optionalKey(
...
/** Compiles updates for trusted writers, including action-owned output fields. */
export function toEffectObjectWriterUpdateSchema(
  object: ObjectType
): Schema.Codec<unknown, unknown> {
  const fields: CompiledSchemaFields = {
    aliases: Schema.optionalKey(recordAliasUpdateSchema),
    ...
    metadata: Schema.optionalKey(metadataSchema),
    ...compileUpdateProperties(object, true),
  }
  return annotateObjectSchema(
    object,
    Schema.Struct(fields),
    `Write ${object.name}`,
    `${pascalCase(object.id)}WriterUpdateInput`
  )
}
