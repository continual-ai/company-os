...
describe("inferred API client", () => {
  it("uses one input object for every object method", async () => {
    ...
    const search = await client.accounts.list({
      filter: {
        and: [
          { field: "name", operator: "contains", value: "example" },
    ...
    expect(calls[4]?.init.body).toBe(
      '{"filter":{"and":[{"field":"name","operator":"contains","value":"example"},{"field":"name","operator":"startsWith","value":"A"}]},"sort":[{"direction":"desc","field":"name","nulls":"last"}]}'
    )
