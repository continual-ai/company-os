...
>(
  ...
  ModuleObjects<SelectModules<TModules, TIds[number]>>,
  ModuleLinks<SelectModules<TModules, TIds[number]>>,
  ModuleInterfaces<SelectModules<TModules, TIds[number]>>,
  TActor
> {
  type Enabled = Model<
    TRoot,
  ...
  const actor: InterfaceType = model.actor
  const root: RootType = model.root
  ...
  for (const module of definition.modules)
    for (const required of module.requires ?? [])
      if (!moduleIds.includes(required))
        throw new Error(`Module '${module.id}' requires module '${required}'.`)
