...
export const seedWorkPerformance = Effect.fn("@company/seedWorkPerformance")(
  function* ({ customers, owners }: CrmSeedData) {
    ...
    for (
      ...
      const project = yield* records.repository(Project).create({
        ...
        status: (
          ["planned", "active", "active", "paused", "completed"] as const
        )[index % 5]!,
    ...
    for (let index = 0; index < Math.ceil(customers.length / 2); index++) {
      ...
      const task = yield* records.repository(Task).create({
        title,
        status,
