...
import { EscalateTicket } from "#/modules/support-engineering/model/index.ts"
import {
  Escalation,
  TicketEscalated,
...
const Input = toEffectSchema(EscalateTicket.input)

/** Escalation owns the receipt, issue and association in one transaction, including concurrent retries. */
export const escalateTicket = Effect.fn("ticket.escalate")(function* (
  input: ActionInput<typeof EscalateTicket>
) {
  ...
  return yield* database.transaction(() =>
    Effect.gen(function* () {
      ...
      if (existing) {
        ...
        if (issue === null)
          return yield* Effect.die("Escalation is missing its required issue.")
        return { issue }
      }
