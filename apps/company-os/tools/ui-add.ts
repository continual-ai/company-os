...
// Adds shadcn components into src/runtime/ui/components through the CLI, then
// makes the result match this repository: `#/` imports get the explicit
// extensions the lint rules require, and `cn` comes from the source-owned
// runtime/ui/lib/utils.ts instead of the `cn` npm package the registry declares.
const app = resolve(dirname(fileURLToPath(import.meta.url)), "..")
const source = join(app, "src")
const components = join(source, "runtime/ui/components")
...
const UTILS = "#/runtime/ui/lib/utils.ts"
...
// The registry declares the `cn` npm package; this repository owns `cn` in
// runtime/ui/lib/utils.ts. Restoring the manifest entry and reinstalling keeps
