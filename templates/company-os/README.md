...
Start the isolated template PostgreSQL service and converge the committed schema:

```sh
pnpm template:setup
```

The runnable template loads its committed non-production `.env.template`, uses port `3202`, and
stores data in the dedicated `company_os_template` database. An instantiated copy instead uses
`apps/company-os/.env.local`, port `3002`, and its own `company_os` database. Local identity mode
works without an external provider. Start the template with `pnpm template:dev`, then choose an
...
converges required records before starting Company OS. Use `pnpm template:dev:all` when the
...
Template database commands load `.env.template` and then the optional ignored
`.env.template.local`. Instantiated-app commands load `apps/company-os/.env.local` when present and
otherwise use `DATABASE_URL` from the process environment. Production supplies the same contract
through deployment environment injection. Only `VITE_` variables may enter browser code; database
and identity assertion configuration remain server-only. Local override files are ignored and
`.env.example` contains no production credentials.
...
1. Edit the source contract in `packages/model`. When registering a new object or interface,
...
2. Remove the existing baseline migration directory and generate a new baseline from the repository
   ...
   ```sh
   pnpm --filter @company-template/company-os db:generate --name initial
   ```
...
4. Validate the migration history and rebuild a fresh database through the integration tests:
   ...
   ```sh
   pnpm --filter @company-template/company-os db:check
   pnpm --filter @company-template/company-os test
   ```

5. Reset the dedicated local database and exercise the affected repository behavior:

   ```sh
   CONFIRM_DATABASE_RESET=company_os_template pnpm --filter @company-template/company-os db:reset
   ```
...
such as `pnpm --filter @company-template/company-os db:generate --name add_company_owner` for every subsequent schema
change.
...
```sh
pnpm --filter @company-template/company-os db:generate:custom --name backfill_company_owner
```
...
```sh
DATABASE_URL="$PRODUCTION_DATABASE_URL" pnpm --filter @company-template/company-os db:deploy
```
...
```sh
pnpm --filter @company-template/company-os dev
```

Open <http://localhost:3202>. Useful endpoints:
