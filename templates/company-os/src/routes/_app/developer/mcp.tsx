    ...
    <div className="mx-auto w-full max-w-[90rem] px-5 py-10 lg:px-8 lg:py-14">
      <section className="max-w-3xl">
        ...
        <p className="mt-3 max-w-2xl leading-7 text-muted-foreground">
          MCP projects the same governed capabilities used by people and
      ...
      <section className="mt-12 grid gap-px border bg-border lg:grid-cols-2">
        <article className="bg-background p-6">
          ...
          <h2 className="mt-8 text-lg font-medium">Streamable HTTP</h2>
          <p className="mt-2 text-sm leading-6 text-muted-foreground">
            Connect to the same deployment at <code>/api/mcp</code>. The
            endpoint uses the configured application identity boundary and
            exposes model queries and actions as governed tools.
          </p>
        </article>
      ...
      <section className="mt-8 border p-6">
        ...
        <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">
          The MCP server establishes the actor, enforces the same authorization
          as HTTP and internal interfaces, derives tools from the model, and
          preserves audit context. It does not implement business behavior
          independently.
        </p>
      </section>
    </div>
