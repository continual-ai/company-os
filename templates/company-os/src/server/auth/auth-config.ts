...
> = Effect.gen(function* () {
  ...
  if (mode === "local") {
    ...
    const local = yield* Config.all({
      cookieName: Config.nonEmptyString("AUTH_LOCAL_COOKIE_NAME").pipe(
        Config.withDefault("company-os-local-identity")
      ),
      email: Config.nonEmptyString("AUTH_LOCAL_EMAIL").pipe(
        Config.withDefault("developer@company.test")
      ),
      issuer: Config.nonEmptyString("AUTH_LOCAL_ISSUER").pipe(
        Config.withDefault("urn:company-os:local")
      ),
